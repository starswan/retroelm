...
import Z80Flags exposing (bit, c_F53, shifter, shifter0, IntWithFlags)
...
--		switch(c & 0xC7) {
   let
      new_r = Bitwise.and (tmp_z80.interrupts.r + 1) 0x7F
      ir_or_r = Bitwise.or tmp_z80.interrupts.ir new_r
      c = m1 tmp_z80.pc (Bitwise.or tmp_z80.interrupts.ir ir_or_r) tmp_z80.env
      env = tmp_z80.env
      ...
      cb_func = group_cb_dict |> Dict.get c.value
   in
     case cb_func of
        Just f -> z80 |> f
        Nothing ->
          if caseval >= 0x40 && caseval <= 0x47 then
                 ...
                 let
                     raw = z80 |> load408bit caseval HL
                     result = Bitwise.and raw.value (1 |> (shiftLeftBy o) |> complement)
                     env_1 = z80.env
                 ...
                 let
                     raw = z80 |> load408bit caseval HL
                     result = Bitwise.or raw.value (1 |> (shiftLeftBy o))
                     env_1 = z80.env
                     x = { z80 | pc = raw.pc, env = { env_1 | time = raw.time } } |> set408bit caseval result HL
                 in
                    Whole x
...
group_xy_cb ixiyhl z80 =
   let
      ...
      o = Bitwise.and (shiftRightBy 3 c.value) 7
--		switch(c&0xC0) {
...
--		}
      v2 = case (Bitwise.and c.value 0xC0) of
         ...
         0x80 -> IntWithFlags (Bitwise.and v1.value (complement (shiftLeftBy o 1))) z80_3.flags
         _ -> IntWithFlags (Bitwise.or v1.value (shiftLeftBy o 1)) z80_3.flags
...
--			case 7: A = v; break;
--		}
      caseval = (Bitwise.and c.value 0x07)
   in
      if caseval /= 6 then
